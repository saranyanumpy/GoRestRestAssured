...
public class CreateInvalidGenderUsersTest extends BaseTest {

    private static final Logger logger = LoggerFactory.getLogger(CreateInvalidGenderUsersTest.class);

    @Test(dataProvider = "invalidUserGender", dataProviderClass = UserDataProvider.class)
    public void testCreateUser(String name, String email, String gender, String status) {
        logger.info("Starting Invalid Gender User test...");
        ...
        // ✅ Validate response
       UserAssertUtil.assertInvalidGenderUserCreated(response, name, email, gender, status);

       
        logger.info("Finished Invalid Gender User test.");
    }
}
